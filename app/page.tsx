...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-10 font-body text-white">
      {/* ================= SCOREBUG ================= */}
        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-6">
            ...
            <div className="text-center">
              <div className="text-xs tracking-widest text-white/60 font-header">
                SUPER BOWL LX
              </div>
              <div
                className={[
                  "mt-1 rounded-md px-3 py-1 text-sm font-bold font-header",
                  isLive
      ...
      <section className="rounded-2xl border border-white/10 bg-white/10 p-6">
        <h2 className="font-header text-xl tracking-tight">
      ...
      <section className="rounded-2xl border border-white/10 bg-black/30 p-6 text-center">
        ...
        <h2 className="mt-2 text-3xl font-header tracking-tight">
          SUPER BOWL SUNDAY
        </h2>
  ...
  return (
    <div className={`flex items-center gap-3 ${align === "right" ? "flex-row-reverse" : ""}`}>
      ...
      <div className={align === "right" ? "text-right" : ""}>
        <div className="text-lg font-header">{name}</div>
...
function TeamComparisonCard({ team }: { team: any }) {
  return (
    ...
    >
      <h3 className="font-header text-xl">{team.name}</h3>
...
function StoryCard({ title, items }: { title: string; items: string[] }) {
  return (
    <div className="rounded-xl border border-white/10 bg-black30 p-4">
      <div className="font-header text-base">{title}</div>
      <ul className="mt-2 list-disc pl-5 text-sm text-white/80">
        {items.map((i) => (
          <li key={i}>{i}</li>
        ))}
